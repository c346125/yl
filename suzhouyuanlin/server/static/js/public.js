...
$(function () {
            ...
            $.ajax({
                ...
                success:function (data) {
                    ...
                    if(data[0].respCode==200){
                        var bht='';
                        data[0].data.forEach(function (v) {
                            bht+='<a href="'+v.url+'">'+v.name+'</a>';
                        });
                        $('#flink').html(bht);
                    }
                }
    ...
    $("#top-search").click(function () {
        ...
        window.location.href = '/list/park?form=' + JSON.stringify(form);
    });
    ...
    $(".loginOut").click(function () {
        dilogboxFun('确定注销吗', function () {
            ...
            $.post('/loginOut')
                .success(function (data) {
                    dilogbox(data[0].respCode, data[0].respMsg, '/login', '', function () {
                        // location.reload()
                    });
                }).error(function (e) {
                console.log(e)
...
function dilogboxs(status, msg, url, twoBtn, callBack) {
    var dom = '';
    ...
    if (status == 200) {
        ...
        dom += '</div>';
        dom += '<div class="layer-btn"><a href="javascript:;" onClick="closedilog();refreshLinkMan();" class="btn blue-bg-down">确定</a></div>';
    } else {
        dom += '<p><i class="tip err"></i></p>' + msg + ''
        dom += '</div>';
        dom += '<div class="layer-btn"><a href="' + (url || 'javscript:;') + '" onClick="closedilog()" class="btn blue-bg-down">确定</a></div>';
    }
    if (twoBtn) {
...
function dilogbox(status, msg, url, twoBtn, callBack) {
    var dom = '';
    dom += '<div class="tips-box">' +
        '<div class="tip-header">提示' +
        <!--i.layer-close X-->
