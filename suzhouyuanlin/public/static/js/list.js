// var imgdomain='https://mallapi.szylly.com/admin/images';
var imgdomain = 'https://mallapi.szylly.com/admin/images';
$(function(){
    var module = $(".html-box").data("module");
	var filterObj={"currPage":1,"pageSize":5};
	var nodes = $('.page-header nav #newsId').data('id')
    if(module==='shop'){
    ...
    if(module==='park'){
	    if(window.location.search){
            var ct=GetQueryString('currentLimiting');
            if(ct==='T'){
                filterObj.currentLimiting=ct;
                $('.page-header nav a').eq(2).addClass('hover').siblings().removeClass('hover');
            }
            //filterObj = JSON.parse(decodeURI(window.location.search.split("=") ));
            // $(".search-put").val(filterObj.parkName);
        }
    }
...
function listAjax(sendData,module) {
    $.ajax({
        ...
        success:function(data){
            listDom(data[0].data.records,module);
        ...
        switch (module){
            ...
            case 'park':
                dom+='<div class="list-item mb10">'+
                        '<div class="item-main">'+
                            '<div class="item-l">'+
                                ' <div class="item-imgs"><a href="/detail/park/'+data[i].goodsCode+'?currentLimiting='+data[i].currentLimiting+'&id='+data[i].id+'"><img src="'+imgdomain+eN(data[i].linkImg)+'" alt=""></a></div>'+
                            '</div>'+
                            '<div class="item-summary">'+
                                '<div class="item-btn-box poa_rt"><a href="/detail/park/'+data[i].goodsCode+'?currentLimiting='+data[i].currentLimiting+'&id='+data[i].id+'" class="btn blue-bgcf">查看更多</a></div>'+
                                '<div class="item-name type0">'+
                                    '<div class="item-tit"><h3><a href="/detail/park/'+data[i].goodsCode+'?currentLimiting='+data[i].currentLimiting+'&id='+data[i].id+'" title="">'+eN(data[i].name || " ")+'</a></h3></div>'+
                                '</div>'+
                                // '<div class="item-info">'+
                                //     '<span class="fl tc"><em class="c-red">'+NN(data[i].salesNum)+' </em>人购买</span><span class="tc"><em class="c-red">'+NN(data[i].evaluateAvgScore)+'</em> / 5分</span><span class="tc"><em class="c-red">'+NN(data[i].evaluateQuantity)+'</em>点评</span>'+
                                // '</div>'+
                                '<div class="item-des">'+eN(data[i].summary)+'</div>'+
                                '<p class="time"><strong>开园时间：</strong>'+eN(data[i].openTime)+'</p>'+
                            '</div>'+
                        '</div>'+
                        '<div class="item-typelist">'+
            ...
            case "news":
                dom+='<div class="m-itemlist mb20">'+
                    ...
                    '<div class="item-name">'+
                    ' <div class="item-tit">'+
                    '<h3><a href="/news/'+data[i].id+'">'+data[i].title+'</a></h3>'+
                    // '<h3><a href='+data[i].link+'>'+data[i].title+'</a></h3>'+
                    '<p><span>创建时间：'+data[i].releaseDate.substr(0,10)+'</span><span>作者：'+data[i].author+'</span></p>'+
                    '</div>'+
                    '</div>'+
                    '<div class="item-summary">'+
                    '<p></p>'+
                ...
                // for(var imgs=0;imgs<imgsdata.length;imgs++){
                    dom+='<a href="/news/'+data[i].id+'" class="img_li">' +
                        '<img src="'+imgdomain+data[i].titleImg+'" alt=""></a>';
                // }
                dom+='</div>'+
                    '</div>'+
                    '</div>'+
